...
export default function CuteGhost3D({ className = "" }) {
  ...
  useEffect(() => {
    ...
    const render = () => {
      ...
      // Cute animated 4 bottom waving ruffles/tentacles
      ...
      // Inner Soft Cyber Rim-Light Shading (Cyan / Purple 3D Depth)
      ...
      // Left cute stubby hand
      ctx.beginPath();
      ctx.fillStyle = "#FFFFFF";
      ctx.ellipse(-52 + idleRotY * 10, 18 + leftHandY, 12, 9, -0.4, 0, Math.PI * 2);
      ctx.fill();

      // Right cute stubby hand
      ctx.beginPath();
      ctx.fillStyle = "#E8EFFF";
      ctx.ellipse(52 + idleRotY * 10, 18 + rightHandY, 12, 9, 0.4, 0, Math.PI * 2);
      ctx.fill();

      // --- 5. Kawaii 3D Eyes & Rosy Cheeks (Curved 3D Surface Mapping & Natural Blinking) ---
      // Periodic blink calculation: blinks every ~3.5 to 4 seconds
      const blinkCycle = (time * 0.9) % 3.5;
      let blinkFactor = 1; // 1 = fully open, 0 = closed
      if (blinkCycle < 0.16) {
        // Fast natural blink curve (close and reopen smoothly)
        blinkFactor = Math.abs(Math.sin((blinkCycle / 0.16) * Math.PI - Math.PI / 2));
      }

      // Clamp and map horizontal rotation onto curved sphere surface
      ...
      // 3D Spherical foreshortening factor for left vs right eye
      ...
      // Rosy Glow Cheeks (Snug on 3D Body)
      ...
      if (blinkFactor < 0.15) {
        ...
        if (blinkFactor > 0.45) {
          // Big Eye Specular Catchlights (Top)
          ctx.fillStyle = "#FFFFFF";
          ctx.beginPath();
